...
// Importámos os ícones Menu e X para o mobile
import { 
...
const isMenuOpen = ref(false); // Controla o menu no mobile

const toggleMenu = () => {
  isMenuOpen.value = !isMenuOpen.value;
};

// Função para navegar e fechar o menu no mobile ao mesmo tempo
const navegar = (rota) => {
  router.push(rota);
...
/* CABEÇALHO MOBILE - Escondido no Desktop */
...
.btn-logout { 
  width: 100%; padding: 12px; border: 1px solid #fee2e2; background: #fff; color: #ef4444; 
  border-radius: 12px; cursor: pointer; font-weight: bold; transition: 0.2s; display: flex; align-items: center; justify-content: center;
}
...
@media (max-width: 992px) {
  ...
  .mobile-header { 
    ...
    position: sticky; top: 0; z-index: 900; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);
  }
  .mobile-logo { height: 40px; object-fit: contain; }
  .btn-mobile-menu { background: transparent; border: none; color: #004aad; cursor: pointer; }

  /* Esconde a barra lateral por defeito e cria a animação de deslizar */

<template>
  <StudentLayout pageTitle="Fórum" pageDescription="Participe das discussões, tire dúvidas e conecte-se com a comunidade.">
        ...
        <div v-else class="glass-card detail-card">
          ...
          <div class="replies-section">
            <h4 class="replies-title">
              <MessageCircle :size="16" /> {{ topicoSelecionado.respostas?.length || 0 }} Resposta(s)
            </h4>
              ...
              <div v-if="replyEmEdicao === reply._id" class="reply-edit-mode">
                <div class="mention-wrapper">
                  ...
                  <div v-if="mostrarSugestoes && sugestoesUsuarios.length > 0" class="mention-dropdown">
                    <div v-for="u in sugestoesUsuarios" :key="u._id" @click="inserirMencao(u.nome)" class="mention-item">
                      <span>{{ u.nome }}</span>
          ...
          <div class="reply-form">
            ...
            <div class="mention-wrapper">
              ...
              <div v-if="mostrarSugestoesResposta && sugestoesUsuarios.length > 0" class="mention-dropdown">
                <div v-for="u in sugestoesUsuarios" :key="u._id" @click="inserirMencaoResposta(u.nome)" class="mention-item">
                  <span>{{ u.nome }}</span>
                  <span :class="['role-badge', u.role]">{{ formatarRole(u.role) }}</span>
                </div>
              </div>
            ...
            <div class="reply-form-footer">
              ...
              <button @click="enviarResposta" class="btn-primary-sm" :disabled="!novaResposta.trim() || enviando">
      ...
      <div class="modal-box">
        ...
        <div class="modal-body">
          <div class="form-group">
            <label>Título</label>
            <input v-model="novoTopico.titulo" placeholder="Qual é a sua dúvida ou sugestão?" class="form-input" />
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <div class="mention-wrapper">
              <textarea v-model="novoTopico.conteudo" placeholder="Descreva o tópico..." class="form-textarea" rows="5" @input="verificarMencaoTopico"></textarea>
              <div v-if="mostrarSugestoesTopico && sugestoesUsuarios.length > 0" class="mention-dropdown">
                <div v-for="u in sugestoesUsuarios" :key="u._id" @click="inserirMencaoTopico(u.nome)" class="mention-item">
                  <span>{{ u.nome }}</span>
                  <span :class="['role-badge', u.role]">{{ formatarRole(u.role) }}</span>
                </div>
              </div>
        ...
        <div class="modal-footer">
          ...
          <button @click="criarTopico" class="btn-primary" :disabled="criando">{{ criando ? 'Criando...' : 'Criar Tópico' }}</button>
        </div>
      </div>
    </div>
...
const topicosFiltrados = computed(() => {
  ...
  return topicos.value.filter(t =>
    t.titulo.toLowerCase().includes(b) || (t.autor?.nome || '').toLowerCase().includes(b)
  );
});

const podeExcluirTopico = computed(() => {
  if (!topicoSelecionado.value) return false;
  return topicoSelecionado.value.autor?._id === userId.value;
});

const podeEditarResposta = (reply) => reply.autor?._id === userId.value;
const podeExcluirResposta = (reply) => reply.autor?._id === userId.value;
...
const carregarUsuarios = async () => {
  try { const res = await api.get('/forum/users'); todosUsuarios.value = res.data; } catch {}
};
...
const enviarResposta = async () => {
  ...
  enviando.value = true;
  try {
    const res = await api.post(`/forum/topics/${topicoSelecionado.value._id}/replies`, { texto: novaResposta.value });
    topicoSelecionado.value = res.data;
    novaResposta.value = '';
...
const iniciarEdicao = (reply) => { replyEmEdicao.value = reply._id; textoEdicao.value = reply.texto; };
const cancelarEdicao = () => { replyEmEdicao.value = null; textoEdicao.value = ''; };

const salvarEdicao = async (replyId) => {
  try {
    const res = await api.put(`/forum/topics/${topicoSelecionado.value._id}/replies/${replyId}`, { texto: textoEdicao.value });
    topicoSelecionado.value = res.data;
...
const buscarSugestoes = (texto) => {
  ...
  if (match) {
    ...
    sugestoesUsuarios.value = todosUsuarios.value.filter(u => u.nome.toLowerCase().startsWith(termo)).slice(0, 6);
    return true;
  }
  return false;
};

const verificarMencao = (e) => { mostrarSugestoes.value = buscarSugestoes(e.target.value); };
const verificarMencaoResposta = (e) => { mostrarSugestoesResposta.value = buscarSugestoes(e.target.value); };
const verificarMencaoTopico = (e) => { mostrarSugestoesTopico.value = buscarSugestoes(e.target.value); };
const inserirMencao = (nome) => { textoEdicao.value = textoEdicao.value.replace(/@\w*$/, `@${nome} `); mostrarSugestoes.value = false; };
const inserirMencaoResposta = (nome) => { novaResposta.value = novaResposta.value.replace(/@\w*$/, `@${nome} `); mostrarSugestoesResposta.value = false; };
const inserirMencaoTopico = (nome) => { novoTopico.value.conteudo = novoTopico.value.conteudo.replace(/@\w*$/, `@${nome} `); mostrarSugestoesTopico.value = false; };
...
const formatarDataCompleta = (d) => d ? new Date(d).toLocaleString('pt-BR') : '';

onMounted(() => { carregarTopicos(); carregarUsuarios(); });
</script>

<style scoped>
.forum-layout { display: grid; grid-template-columns: 320px 1fr; gap: 25px; align-items: start; }
.glass-card { background: white; border-radius: 20px; border: 1px solid #e2e8f0; box-shadow: 0 4px 20px rgba(15,23,42,0.05); padding: 25px; }
.col-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; }
.section-title { font-size: 1rem; font-weight: 800; color: #1e293b; margin: 0; display: flex; align-items: center; gap: 8px; }
.text-brand { color: #004aad; }
.search-box { position: relative; margin-bottom: 15px; }
.search-icon { position: absolute; left: 12px; top: 50%; transform: translateY(-50%); color: #94a3b8; }
.search-input { width: 100%; padding: 10px 12px 10px 36px; border: 1px solid #e2e8f0; border-radius: 10px; background: #f8fafc; font-size: 0.9rem; color: #1e293b; box-sizing: border-box; }
.search-input:focus { outline: none; border-color: #004aad; background: white; }
.loading-state { display: flex; align-items: center; gap: 10px; color: #94a3b8; padding: 20px 0; justify-content: center; }
.empty-state { display: flex; flex-direction: column; align-items: center; gap: 10px; padding: 30px 0; color: #94a3b8; text-align: center; }
.opacity-10 { opacity: 0.1; } .opacity-20 { opacity: 0.2; }
.topics-list { display: flex; flex-direction: column; gap: 8px; max-height: calc(100vh - 300px); overflow-y: auto; }
.topics-list::-webkit-scrollbar { width: 5px; } .topics-list::-webkit-scrollbar-thumb { background: #e2e8f0; border-radius: 10px; }
.topic-item { padding: 14px; border: 1px solid #e2e8f0; border-radius: 12px; cursor: pointer; transition: all 0.2s; }
.topic-item:hover { border-color: #bfdbfe; background: #f8fafc; }
.topic-item.active { border-color: #004aad; background: #eff6ff; }
.topic-item-top { display: flex; justify-content: space-between; align-items: flex-start; gap: 8px; margin-bottom: 8px; }
.topic-title { font-weight: 700; font-size: 0.9rem; color: #1e293b; line-height: 1.3; }
.topic-date { font-size: 0.72rem; color: #94a3b8; white-space: nowrap; }
.topic-item-bottom { display: flex; justify-content: space-between; align-items: center; }
.topic-author, .topic-replies { display: flex; align-items: center; gap: 4px; font-size: 0.75rem; color: #64748b; }
.empty-detail { display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 12px; min-height: 400px; color: #94a3b8; }
.detail-card { display: flex; flex-direction: column; gap: 20px; }
.detail-header { border-bottom: 1px solid #f1f5f9; padding-bottom: 20px; }
.detail-title-row { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 10px; }
.detail-title-row h2 { font-size: 1.3rem; font-weight: 800; color: #0f172a; margin: 0; line-height: 1.3; }
.detail-actions { display: flex; gap: 8px; flex-shrink: 0; }
.detail-meta { display: flex; align-items: center; gap: 20px; flex-wrap: wrap; margin-bottom: 15px; }
.meta-author, .meta-date { display: flex; align-items: center; gap: 5px; font-size: 0.8rem; color: #64748b; }
.detail-content p { font-size: 0.95rem; color: #374151; line-height: 1.6; margin: 0; }
.replies-section { display: flex; flex-direction: column; gap: 12px; }
.replies-title { font-size: 0.9rem; font-weight: 800; color: #475569; display: flex; align-items: center; gap: 6px; margin: 0 0 5px 0; }
...
.reply-body p { font-size: 0.9rem; color: #374151; line-height: 1.6; margin: 0; }
.edited-tag { font-size: 0.68rem; color: #94a3b8; margin-left: 6px; font-style: italic; }
.reply-form { border-top: 1px solid #f1f5f9; padding-top: 20px; }
.reply-form-title { font-size: 0.9rem; font-weight: 800; color: #475569; display: flex; align-items: center; gap: 6px; margin: 0 0 12px 0; }
.reply-textarea { width: 100%; padding: 12px; border: 1px solid #e2e8f0; border-radius: 12px; background: #f8fafc; font-size: 0.9rem; color: #1e293b; resize: vertical; font-family: inherit; line-height: 1.5; box-sizing: border-box; }
.reply-textarea:focus { outline: none; border-color: #004aad; background: white; }
.reply-form-footer { display: flex; justify-content: space-between; align-items: center; margin-top: 10px; }
.mention-hint { font-size: 0.75rem; color: #94a3b8; display: flex; align-items: center; gap: 4px; }
.reply-edit-mode { margin-top: 10px; }
.edit-actions { display: flex; gap: 8px; justify-content: flex-end; margin-top: 8px; }
.mention-wrapper { position: relative; }
.mention-dropdown { position: absolute; top: 100%; left: 0; right: 0; background: white; border: 1px solid #e2e8f0; border-radius: 10px; box-shadow: 0 8px 20px rgba(0,0,0,0.1); z-index: 100; max-height: 200px; overflow-y: auto; }
.mention-item { padding: 10px 14px; cursor: pointer; display: flex; align-items: center; gap: 8px; font-size: 0.85rem; color: #1e293b; font-weight: 600; }
...
.btn-secondary-sm { background: white; color: #64748b; border: 1px solid #e2e8f0; padding: 7px 14px; border-radius: 10px; font-weight: 600; font-size: 0.85rem; cursor: pointer; transition: 0.2s; }
.btn-secondary-sm:hover { background: #f8fafc; }
.btn-icon { background: transparent; border: 1px solid #e2e8f0; border-radius: 8px; padding: 6px; cursor: pointer; color: #64748b; display: flex; align-items: center; transition: 0.2s; }
.btn-icon:hover { background: #f1f5f9; }
.btn-icon.btn-danger:hover { background: #fee2e2; color: #ef4444; border-color: #fecaca; }
.btn-icon-sm { background: transparent; border: none; padding: 4px; cursor: pointer; color: #94a3b8; border-radius: 6px; display: flex; align-items: center; transition: 0.2s; }
.btn-icon-sm:hover { color: #475569; background: #f1f5f9; }
.btn-danger-sm:hover { color: #ef4444 !important; background: #fef2f2 !important; }
.modal-overlay { position: fixed; inset: 0; background: rgba(15,23,42,0.5); display: flex; align-items: center; justify-content: center; z-index: 2000; backdrop-filter: blur(2px); }
.modal-box { background: white; border-radius: 24px; width: 100%; max-width: 560px; margin: 20px; overflow: hidden; }
...
.modal-header h3 { font-size: 1.1rem; font-weight: 800; color: #1e293b; margin: 0; display: flex; align-items: center; gap: 8px; }
.modal-body { padding: 24px 28px; display: flex; flex-direction: column; gap: 16px; }
.modal-footer { padding: 20px 28px; border-top: 1px solid #f1f5f9; display: flex; justify-content: flex-end; gap: 12px; }
...
.form-input, .form-select, .form-textarea { padding: 12px; border: 1px solid #e2e8f0; border-radius: 10px; background: #f8fafc; font-size: 0.9rem; color: #1e293b; font-family: inherit; box-sizing: border-box; width: 100%; }
.form-input:focus, .form-select:focus, .form-textarea:focus { outline: none; border-color: #004aad; background: white; }
.form-textarea { resize: vertical; line-height: 1.5; }
.btn-primary { background: #004aad; color: white; border: none; padding: 12px 24px; border-radius: 12px; font-weight: 700; cursor: pointer; transition: 0.2s; }
.btn-primary:hover:not(:disabled) { background: #003a8c; }
.btn-primary:disabled { opacity: 0.5; cursor: not-allowed; }
.btn-secondary { background: white; color: #64748b; border: 1px solid #e2e8f0; padding: 12px 20px; border-radius: 12px; font-weight: 600; cursor: pointer; }
.spin { animation: spin 1s linear infinite; }
@keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
:deep(.mencao) { color: #004aad; font-weight: 700; background: #eff6ff; padding: 1px 4px; border-radius: 4px; }

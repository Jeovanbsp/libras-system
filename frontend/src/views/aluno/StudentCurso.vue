      ...
      <div v-for="c in cursos" :key="c._id" class="glass-card course-card">
        ...
        <div class="course-footer">
          <span class="meta-info">
            <Clock :size="16" class="meta-icon" /> {{ c.cargaHoraria }}h de carga
          </span>
          <button @click="acessarCurso(c._id)" class="btn-access">
            Acessar Aula
          </button>
        </div>
      </div>
...
import { BookOpenCheck, Clock, Inbox } from 'lucide-vue-next';
...
const buscarMeusCursos = async () => {
  try {
    ...
    console.error("Erro ao carregar seus cursos");
  }
};
